...
function DomainCard({ domain, base }) {
  ...
  return (
    ...
    >
      <div className={`h-1 ${colors.bg.replace('bg-', 'bg-').replace('-50', '-400')}`} />
      <div className="p-4">
...
export default function DomainsPage() {
  ...
  return (
    <div>
      <div className="mb-4">
        <h1 className="text-lg font-semibold text-gray-900">Architecture Domains</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-gray-200 border border-gray-200">
        {DOMAINS.map(domain => (
          <div key={domain.id} className="bg-white">
            <DomainCard domain={domain} base={base} />
          </div>
        ))}
      </div>
    </div>
