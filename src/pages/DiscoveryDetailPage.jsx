        ...
        <div className="mb-6 pb-5 border-b border-gray-200">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {discovery.scope && <div className="mt-2"><ScopeChip scope={discovery.scope} /></div>}
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 self-start">
              <span className={`text-xs font-medium px-2 py-1 capitalize ${STATUS_BADGE[discovery.status] ?? 'bg-gray-100 text-gray-600'}`}>
                {discovery.status}
              </span>
              <span className="text-xs font-mono bg-gray-100 text-gray-500 px-2 py-1">{discovery['discovery-id']}</span>
            </div>
          </div>
        </div>
