...
async function syncIndex(
  ...
  await writeFile(
    iPath,
    index,
    `Sync ${decisionId} in decisions index (${status})`,
    sha,
    BASE,
    token,
    owner,
...
async function updateDecision({ clientId, versionId, decisionId, updates }, token, owner, repo) {
  ...
  if (newStatus === 'rejected') {
    ...
    await syncIndex(
      ...
      versionId,
      decisionId,
      { title: updated.title, status: 'rejected', scope: updated.scope },
      token,
      owner,
      repo
    )
  ...
  await writeFile(
    dPath,
    updated,
    `Update ${decisionId}: status → ${newStatus ?? 'updated'}`,
    sha,
    branch,
    token,
    owner,
  ...
  if (newStatus) {
    await syncIndex(
      ...
      versionId,
      decisionId,
      { title: updated.title, status: newStatus, scope: updated.scope },
      token,
      owner,
      repo
    )
  ...
  if (newStatus === 'proposed') await dispatch('decisions-to-proposed.yml', ids, token, owner, repo)
    ...
    await dispatch('decisions-to-accepted.yml', ids, token, owner, repo)
...
async function updateFinding(
  { clientId, versionId, decisionId, sectionKey, findingIndex, review },
  token,
  owner,
  repo
) {
  ...
  let content, sha, ref
  try {
    ;({ content, sha } = await readFile(dPath, branch, token, owner, repo))
    ref = branch
  } catch {
    ;({ content, sha } = await readFile(dPath, BASE, token, owner, repo))
    ref = BASE
  }
...
async function editDecision(
  { clientId, versionId, decisionId, title, context, problem, proposal, requirements, scope },
  token,
  owner,
  repo
) {
...
async function commitDecision({ clientId, versionId, decisionId, prNumber }, token, owner, repo) {
  ...
  if (prNumber) {
    await deleteBranch(decisionBranch(clientId, versionId, decisionId), token, owner, repo).catch(
      () => {
        /* branch may already be gone if delete-on-merge is enabled */
      }
    )
  }
  return { ok: true, decisionId, status: 'committed' }
...
async function syncDiscoveryIndex(
  ...
  const { content: index, sha } = await readDiscoveriesIndex(
    clientId,
    versionId,
    token,
    owner,
  ...
  await writeFile(
    iPath,
    index,
    `Sync ${discoveryId} in discovery index (${entry.status})`,
    sha,
    BASE,
    token,
    owner,
...
export default async function handler(req, res) {
  ...
  if (!token || !owner || !repo) {
    return res.status(503).json({
      ...
      missing: [!token && 'GITHUB_TOKEN', !owner && 'GITHUB_OWNER', !repo && 'GITHUB_REPO'].filter(
        Boolean
      ),
    })
  }
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'edit-decision')
        return res.json(await editDecision(params, token, owner, repo))
      if (action === 'update-decision')
        return res.json(await updateDecision(params, token, owner, repo))
      if (action === 'update-finding')
        return res.json(await updateFinding(params, token, owner, repo))
      if (action === 'commit-decision')
        return res.json(await commitDecision(params, token, owner, repo))
