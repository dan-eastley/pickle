...
export default function ScopeSelector({ domain, abstraction, artefact, onChange }) {
  ...
  function setDomain(value) { onChange({ domain: value, abstraction: '', artefact: '' }) }
  function setAbstraction(value) { onChange({ domain, abstraction: value, artefact: '' }) }
  function setArtefact(value) { onChange({ domain, abstraction, artefact: value }) }
  function clear() { onChange({ domain: '', abstraction: '', artefact: '' }) }
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <select value={abstraction} onChange={e => setAbstraction(e.target.value)}
      ...
      <select value={artefact} onChange={e => setArtefact(e.target.value)}
        disabled={!domain} className={`${selectClass} disabled:opacity-40`}>
      ...
      {(domain || abstraction || artefact) && (
        <button onClick={clear} className="text-xs text-gray-400 hover:text-gray-600 transition-colors px-2 py-1">
          Clear
        </button>
      )}
    </div>
