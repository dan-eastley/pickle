...
function countSummary(items, schema, data) {
  ...
  if (meta?.renderAs === 'grouped' && meta?.grouping) {
    ...
    const pLabel =
      (pCount === 1 ? labels?.parent : labels?.parent ? labels.parent + 's' : 'groups') ?? 'groups'
    const cLabel =
      (cCount === 1 ? labels?.child : labels?.child ? labels.child + 's' : 'items') ?? 'items'
...
function CellValue({ value, colDef }) {
  ...
  if (colDef.enum) {
    const variantMap = {
      ...
      supporting: 'blue',
      management: 'gray',
      'system-of-record': 'gray',
      'system-of-engagement': 'blue',
      'system-of-insight': 'violet',
      'system-of-innovation': 'amber',
      entity: 'blue',
      event: 'amber',
      reference: 'gray',
      public: 'success',
    ...
    return (
      <ul className="space-y-0.5">
        {value.slice(0, 3).map((v, i) => (
          <li
            key={i}
            className="text-xs text-gray-600 before:content-['•'] before:mr-1.5 before:text-gray-300"
          >
            {String(v)}
          </li>
        ))}
  ...
  if (colDef.key === 'id') {
    return (
      <span className="font-mono text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5">{value}</span>
    )
  }
...
export default function CatalogueView({ data, schema, onOpenEntity }) {
  ...
  if (grouped && meta.grouping) {
    ...
    function expandAllGrouped() {
      setExpanded(new Set(groupedData.map((p) => p.id)))
    }
    function collapseAllGrouped() {
      setExpanded(new Set())
    }
