...
Each client version is modelled across five architecture domains. Every artefact is aligned to exactly one domain.

| Domain | Folder | Acronym | Description |
|---|---|---|---|
| **Business Architecture** | `business/` | `BUS` | Captures the organisation's capabilities, processes, and operating model. Defines what the business does and why, providing the context that all other domains serve. |
...
| **Solution Architecture** | `solution/` | `SOL` | Cross-cutting designs that span multiple domains to address a specific business problem or initiative. Brings together business, data, integration, and application concerns into a coherent delivery blueprint. |
