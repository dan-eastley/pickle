...
| Field | Description |
|---|---|
| `finding` | What you observed. Name artefact types by their full name and ID. |
| `impact` | What this means for the decision — which catalogues will need editing, what is at risk if they are not. |
| `recommendation` | What the author should do: which artefact types to update, add, or check. Be specific. |
| `rationale` | Why this is the right course of action — ground it in business outcomes or governance. |

Keep every field tight — **≤300 characters**, a crisp statement or a few short bullets, never a paragraph. Use **markdown** in each field — bold artefact names, use `code spans` for IDs, and bullet lists where multiple items need enumerating.
