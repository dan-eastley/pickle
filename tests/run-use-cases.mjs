...
const CHECKS = {
  "Browse the architecture by domain": async (page) => {
    ...
    if (await page.getByText("This page failed to load.").count())
      return fail("error boundary tripped");
    ...
    const heading = await page
      ...
      .innerText()
      .catch(() => "");
    if (!/Architecture/i.test(heading))
      return fail(`domain page heading unexpected: "${heading}"`);
...
function rendersWithContent(path, mustText) {
  return async (page) => {
    ...
    if (mustText && !(await page.getByText(mustText).count()))
      return fail(`expected to see "${mustText}"`);
    const bodyLen = (
      await page
        .locator("main")
        .innerText()
    ...
    return pass(
      mustText ? `renders with "${mustText}"` : `renders (${bodyLen} chars)`,
    );
  };
}

Object.assign(CHECKS, {
  "Drill into an abstraction layer": rendersWithContent(
    "domains/business/conceptual",
  ),
  "View a business capability catalogue": rendersWithContent(
    "domains/business/conceptual/BUS-CAP",
    "Customer Management",
  ),
  "View a data domains & concepts catalogue": rendersWithContent(
    "domains/data/conceptual/DAT-DAC",
  ),
  "View principles and guardrails": rendersWithContent(
    "domains/business/logical/BUS-PRN",
  ),
  "View the application landscape": rendersWithContent(
    "domains/application/logical/APP-DAP",
  ),
  "View the business capability model": rendersWithContent(
    "domains/business/conceptual/BUS-BCM",
    "Customer Management",
  ),
  "View the business process model": rendersWithContent(
    "domains/business/conceptual/BUS-BPM",
  ),
  "Read an architecture vision": rendersWithContent(
    "domains/solution/conceptual/SOL-AVI",
  ),
  "Read a solution intent": rendersWithContent(
    "domains/solution/logical/SOL-SVI",
  ),
  "Read an interface specification": rendersWithContent(
    "domains/solution/physical/SOL-ISP",
  ),
  "See an artefact’s change history": rendersWithContent(
    "domains/business/conceptual/BUS-CAP",
    "Joe B",
  ),
  "Export the raw JSON of an artefact": rendersWithContent(
    "domains/business/conceptual/BUS-CAP",
    "BUS-CAP.json",
  ),
  "View a capability-to-process matrix": rendersWithContent(
    "domains/business/conceptual/BUS-CAP-PRO",
  ),
  "View a capability-to-application matrix": rendersWithContent(
    "domains/application/logical/APP-CAP-DAP",
  ),
  "Follow a related-artefact link": rendersWithContent(
    "domains/business/conceptual/BUS-CAP",
    "Related Artefacts",
  ),
  "Raise an architecture decision": rendersWithContent(
    "decisions/new",
    "New Architecture Decision",
  ),
  "Raise an architecture discovery": rendersWithContent(
    "discovery/new",
    "New Architecture Discovery",
  ),
  "Review past discoveries": rendersWithContent("discovery", "Active"),
  "Filter decisions by scope": rendersWithContent(
    "decisions?domain=business",
...
console.log(
  `Running ${selected.length} use case(s) — complexity=${FILTER_C}, priority=${FILTER_P}\n`,
);
  ...
  const icon =
    outcome.status === "passed"
      ...
      : outcome.status === "failed"
        ...
        : "[TODO]";
  console.log(
    `${icon} ${uc.id} ${uc.title} (${uc.actor}) — ${outcome.status}: ${outcome.note}`,
  );
}
...
const tally = (key) => {
  ...
  for (const u of selected) {
    ...
    m[k][u.outcome.status]++;
    m[k].total++;
  }
  return m;
};
const statusTotals = selected.reduce(
  (m, u) => {
    m[u.outcome.status]++;
    m.total++;
    return m;
  },
  { passed: 0, failed: 0, todo: 0, total: 0 },
);

const table = (title, m) => {
  let s = `### By ${title}\n\n| ${title} | Passed | Failed | To do | Total |\n|---|---|---|---|---|\n`;
  for (const [k, v] of Object.entries(m))
    s += `| ${k} | ${v.passed} | ${v.failed} | ${v.todo} | ${v.total} |\n`;
  return s + "\n";
};
...
for (const u of selected) {
  const icon =
    u.outcome.status === "passed"
      ? "✅ passed"
      : u.outcome.status === "failed"
        ? "❌ failed"
        : "⬜ to do";
...
if (failures.length) {
  ...
  for (const f of failures)
    console.log(`  ${f.id} ${f.title} — ${f.outcome.note}`);
}
