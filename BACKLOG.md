...
### ✅ L1 — Schema validation · High
**Context:** `tests/validate-schemas.mjs` (Ajv) loads every schema by `$id` (cross-file scope/roles URN `$refs` resolve), compiles them, and validates every instance declaring a `$schema` URN. Runs in `ci.yml` → `validate-data` on data/schema/config changes. Caught and fixed three latent bugs on introduction.

### ✅ L2 — Referential integrity · Medium
**Context:** `tests/validate-integrity.mjs` checks clients/versions/discovery index↔folder consistency, `audience`/`author` roles vs `roles.json`, `$schema` URN↔path mirroring, and content-array presence. Runs alongside L1.

### ✅ Path-scoped CI · Medium
**Context:** `ci.yml` uses a `changes` (paths-filter) job so only the touched areas run — lint/unit/build for `src/**`, validate-data for data/schema/config. Browser tests run against the Vercel deployment (`post-deploy.yml`); nightly full corpus + screenshots (`nightly.yml`). Superseded `test.yml`.
**Gap:** The dormant `validate-context.yml` (change-surface checks) is still `if: false` — fold into validate-data or retire.
...
**Proposed fix:** Run Prettier across the tree and add `format:check` to CI; keep widening use-case checks (63 still "to do").
...
**Context:** `tests/use-cases.json` — 100 use cases, every role/complexity/priority covered. `tests/run-use-cases.mjs` drives them against the deployment and records outcomes in the JSON + `tests/use-case-outcomes.md`. Latest baseline: **37 passed, 0 failed, 63 to-do**. Runs on the XS/Must-Have subset post-deploy and the full corpus nightly.
**Gap:** 63 titles still need an automated check (currently "to do"); failures auto-route to the *Use Case Outcomes* epic.

### ✅ URL-mapped screenshots + route smoke · Low
**Context:** `screenshot.mjs` mirrors the URL structure; `smoke.spec.js` hits all 66 routes against the deployment; `nightly.yml` regenerates screenshots as an artifact.
